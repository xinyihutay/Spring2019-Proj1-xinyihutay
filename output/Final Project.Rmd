...
Happiness is the emotion that we all share. However, things that make us happy might be relatively different from each other. Therefore, this project aims to analyze whether gender, marital status, parenthood and cultural background would differ the things and events that make people feel happy.
...
word = c("happy","ago","yesterday","lot","today","months","month",
                 "happier","happiest","last","week","past","happniess")
...
## Part 2: Relationships between word
...
So I visualized some details of the text structure. We can see that words like **visit**, **party** and **school** are often followed by the word **friend**, which means when these events are related to friends, people usually get happy. 
...
Now in this part, I applied topic modeling to all the happy moments corpuses. I set the number of topics to be 9, which is the same as the number of 'topic_dict' in HappyDB data folder.
...
According to the above plots and the words relationships we just found out, I sort the 9 topics as follows:
...
The wordclouds look good, but except for 'day', 'time' and 'friend', we can hardly see other words clearly. So let's check these popular words by using ggplot.

```{r  Gender freq ggplot}
...
In both plots, the first 3 words are 'friend', 'day' and 'time'. Their frequencies are almost equivalent in female data set, while in male data set, the word frequency of ‘friend’ is obviously higher than the other two words. And the fourth words in both data set are spouse, which is good. 

In the male data set, words 'played', 'watched' rank higher than family. But in female data set, words related to family and home are in the front. So you can see the differences of happy moments between female and male.

```{r male tm,warning=FALSE,error=FALSE,echo=FALSE,message=FALSE}
index_male = which(hm.demographic$gender == "m")
comb_male = vector(mode = "character",
                   length = length(unique(hm.demographic$wid[index_male])))

for (i in sort(unique(hm.demographic$wid[index_male]))) {
  index = which(hm.demographic$wid == i)
  comb_male[i] = paste(hm.demographic$text[index], collapse = " ")
}

dtm.male = DocumentTermMatrix(VCorpus(VectorSource(comb_male)))
rowTotals.male = apply(dtm.male, 1, sum) 
dtm.male  = dtm.male[rowTotals.male>0, ]

male_lda = LDA(dtm.male, k = 9, control = list(seed = 1234))
male_topics = tidy(male_lda, matrix = "beta")
...
```{r female tm,warning=FALSE,error=FALSE,echo=FALSE,message=FALSE}
index_female = which(hm.demographic$gender == "f")
comb_female = vector(mode = "character",
                   length = length(unique(hm.demographic$wid[index_female])))

for (i in sort(unique(hm.demographic$wid[index_female]))) {
  index = which(hm.demographic$wid == i)
  comb_female[i] = paste(hm.demographic$text[index], collapse = " ")
}

dtm.female = DocumentTermMatrix(VCorpus(VectorSource(comb_female)))
rowTotals.female = apply(dtm.female, 1, sum) 
dtm.female  = dtm.female[rowTotals.female>0, ]

female_lda = LDA(dtm.female, k = 9, control = list(seed = 1234))
female_topics = tidy(female_lda, matrix = "beta")

female_top_terms = female_topics %>%
  group_by(topic) %>%
  top_n(10, beta) %>%
  ungroup() %>%
  arrange(topic, -beta)
...
While in the no-child cluster, except for words **day**, **time** and **friends**, we can see words related to entertainment show up frequently, such as **game**, **played** and **watched**.
...
In the Asia cluster, we only see one word which is kind of related to entertainment, that is enjoyed. It seems like And many Asian workers described their happy moments related to the word birthday which only appears in Asia cluster.  

The first 10 popular words of workers from Northern America are somewhat similar to that of Europe. They all contain a couple of  words related to entertainments like won played watched, and these words rank relatively high. But in terms of people and family, Europe cluster contains word girlfriend and then the word family, while Northern America cluster contains words home and daughter. Besides, word dinner shows up in Northern America cluster, it might suggest that dinner is something important in Northern American culture.
...
However, as you can see in the y-lab, it reveals that the number of workers from Africa, Southern America and Oceania is quite small, Oceania in particular. So the analysis on popular words in these two clusters is not very accurate.
