...
urlfile<-'https://raw.githubusercontent.com/rit-public/HappyDB/master/happydb/data/cleaned_hm.csv'  # Cleaned_hm file
...
```{r tidy dictionary}
dict <- tidy(corpus) %>%  # convert tm to tidy 
  select(text) %>%
...
Lastly, we complete the stems by picking the corresponding word with the highest frequency.  **why do we need to count the highest freq ?**
...
```{r reverse unnest}
completed <- completed %>%
  group_by(id) %>%  # sort the correct words according to each id
  summarise(text = str_c(word, collapse = " ")) %>%  # combine the words
  ungroup() 
```
...
hm_data <- hm_data %>%
  mutate(id = row_number()) %>%   # matching the correct id to the original file
